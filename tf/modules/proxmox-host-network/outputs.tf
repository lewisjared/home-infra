...
output "vlan_interfaces" {
  ...
  value = {
    for key, vlan in proxmox_network_linux_vlan.host_vlan : key => {
      host    = vlan.node_name
      name    = vlan.name
      vlan_id = vlan.vlan
...
output "vlan_interfaces_by_host" {
  ...
  value = {
    for host_name in distinct([for v in proxmox_network_linux_vlan.host_vlan : v.node_name]) :
    host_name => {
      for key, vlan in proxmox_network_linux_vlan.host_vlan :
      "vlan${vlan.vlan}" => {
        name    = vlan.name
        address = vlan.address
      }
